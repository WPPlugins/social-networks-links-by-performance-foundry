...
class Foundry_Social_Links_Front_End {
	...
	public function the_social_networks( $display_name ) {
		...
		<div class="foundry-social">
			...
			foreach ( $this->social_networks as $network ) {
				$fa_icon = $name = $name_class = $icon_name_class = $icon_color = '';
				...
				if ( ! empty( $network['url'] ) ) {
					...
					if ( $this->use_fa_icon ) {
						$fa_icon = '<i class="fa ' . esc_html( $network['icon'] ) . ' foundry-social__icon ' . $icon_name_class . '" aria-hidden="true"></i>';
					}
					...
					echo '<a ' . wp_kses_post( $icon_color ) . ' class="foundry-social__link ' . esc_attr( $network['custom_class'] ) . ' ' . esc_attr( $name_class ) . ' " href="' . esc_url( $network['url'] ) . '">' . wp_kses_post( $fa_icon ) . esc_html( $name ) . '</a>';
				}
			}
			?>
			</div>
	...
	public function shortcode( $atts ) {
		...
		$atts = shortcode_atts( array(
			'display_name' => false,
		), $atts, 'foundry_social_links' );

		if ( 'true' === $atts['display_name'] ) {
			$display_name = true;
		}

		return $this->the_social_networks( $display_name );
